...
```{r}
epadata = data.frame(read.csv('~/Desktop/20Summer/Stat420/Week7/epa2017.csv'))
co2vhorse = lm(CO2 ~ horse, data = epadata)
library(ggplot2)
ggplot(aes(x = CO2, y = horse, color = type), data = epadata) + geom_point() + geom_abline(intercept = coef(summary(co2vhorse))[1,1], slope = coef(summary(co2vhorse))[2,1])
```
...
```{r}
coef(summary(co2vhorse))["horse",1]
```
...
predict(co2vhorse, impreza_wagon, interval = "prediction", level = 0.90)
```
...
- Make a scatterplot of `CO2` versus `horse`. Use a different color point for each vehicle `type`.
```{r}
b = ggplot(aes(x = CO2, y = horse, color = type, group = type), data = epadata) + geom_point()
```

- Fit an additive multiple regression model with `CO2` as the response and `horse` and `type` as the predictors.
```{r}
co2horsetype = lm(CO2~0+horse+type, data = epadata)
summary(co2horsetype)$coef[,1]
```
...
```{r}
b + geom_abline(aes(intercept = coef(summary(co2horsetype))[2,1], slope = coef(summary(co2horsetype))[1,1])) + geom_abline(aes(intercept = coef(summary(co2horsetype))[3,1], slope = coef(summary(co2horsetype))[1,1])) + geom_abline(aes(intercept = coef(summary(co2horsetype))[4,1], slope = coef(summary(co2horsetype))[1,1]))
```
- Give an estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car`. 
```{r}
coef(summary(co2horsetype))[1,1]
```
...
```{r}
predict(co2horsetype, impreza_wagon, interval = "prediction", level = 0.90)
```
...
- Make a scatterplot of `CO2` versus `horse`. Use a different color point for each vehicle `type`.
```{r}
c = ggplot(aes(x = CO2, y = horse, color = type), data = epadata) + geom_point()
```

- Fit an interaction multiple regression model with `CO2` as the response and `horse` and `type` as the predictors.
```{r}
co2horsetypeinter = lm(CO2 ~ 0 + type + horse:type, data = epadata)
summary(co2horsetypeinter)
```

- Add the fitted regression "lines" to the scatterplot with the same colors as their respective points (one line for each vehicle type). Comment on how well this line models the data.
```{r}
c + geom_abline(aes(intercept = coef(summary(co2horsetypeinter))[1,1], slope = coef(summary(co2horsetypeinter))[4,1])) + geom_abline(aes(intercept = coef(summary(co2horsetypeinter))[2,1], slope = coef(summary(co2horsetypeinter))[5,1])) + geom_abline(aes(intercept = coef(summary(co2horsetypeinter))[3,1], slope = coef(summary(co2horsetypeinter))[6,1]))
```

- Give an estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car`. 
```{r}
coef(summary(co2horsetypeinter))[5,1]
```

- Give a 90% prediction interval using this model for the `CO2` of a Subaru Impreza Wagon, which is a vehicle with 148 horsepower and is considered type `Both`. 
```{r}
predict(co2horsetypeinter, impreza_wagon, interval = "prediction", level = 0.90)
```
...
```{r}
anova(co2horsetype, co2horsetypeinter)
```
***
...
```{r}
hospital = data.frame(read.csv('~/Desktop/20Summer/Stat420/Week7/hospital.csv'))
str(hospital)
...
```{r}
hospitaladd = lm(Blood ~ Age + Education + Income + Sex, data = hospital)
coef(hospitaladd)
```
The reference level is Low for Education, low for income, male for Sex.
...
```{r}
hospitalint = lm(Blood ~ (Age + Income) * Sex + Education , data = hospital)
anova(hospitaladd,hospitalint)[2,]$"Pr(>F)"
```
Because the p-value is greater than alpha fail to reject null hypothesis meaning, prefer additive.
...
```{r}
hospitalthree = lm(Blood ~ Age * Income * Sex + Education + 0 , data = hospital)
anova(hospitaladd,hospitalthree)[2,]$"Pr(>F)"
```
Since p value is lower than alpha, reject the null hypothesis and prefer three interaction.
...
```{r}
coef(summary(hospitalthree))[9,1]
```
Estimate of change in average blood would be -0.1345.
...
Null hypothesis, there is no relationship among Pressure, blood and rate. $\beta_4 = \beta_5 = \beta6 = \beta_7 = 0$
Alternative Hypothesis, there are some interaction among Pressure, blood, and rate. $\beta_4,\beta_5, \beta_6, \beta_7 \neq 0$
...
Because p value is less than 0.10 reject the null hypothesis.
- Which model you prefer
I would prefer interactive model.
...
Y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \beta_4 x_1 x_2 + \beta_5 x_1 x_3 + \beta_6 x_2 x_3 + \beta_7 x_1 x_2 x_3 + \epsilon,
\]
$x_3 = rate$ , the corresponding change occurs in $\beta_3, \beta_5, \beta_6, \beta_7$
Therefore, the linear function would be like $Y = \beta_3 + 10\beta_5 + 139\beta_6 + 1390\beta_7$$
...
\] , the difference is in $\beta_3$. Therefore the linear function of the difference would be $Y = \beta_3$
...
all.equal(lm_t, -tt_t)
```
Because one is negative due to the dummy variable that R sets.
